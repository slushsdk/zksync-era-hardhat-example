...
describe("Greeter", () => {
  const deployGreeter = async (): Promise<{ greeterContract: Contract, initialGreeting: string }> => {
    // tests should run on local testnet with a rich wallet's private key
    if (hre.network.name !== "zkSyncLocal") {
      throw new Error("Tests triggered on non-local testnet network. Use the --network zkSyncLocalTestnet flag for local testing")
    }
    const privateKey = process.env.LOCAL_TESTNET_RICH_WALLET_PRIVATE_KEY
    if (!privateKey) {
      throw new Error(`Please set your LOCAL_TESTNET_RICH_WALLET_PRIVATE_KEY in the '.env' file. Use the '.env.example' file as an example.`);
    }
